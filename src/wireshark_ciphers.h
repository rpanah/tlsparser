...
const char * get_cipher_name(unsigned int code)
{
    unsigned length = sizeof(ssl_31_ciphersuite) / sizeof(ssl_31_ciphersuite[0]);
    unsigned i = 0;
    for (i = 0; i < length; i++)
        if (ssl_31_ciphersuite[i].value == code)
            return ssl_31_ciphersuite[i].strptr;
    return NULL;
}
